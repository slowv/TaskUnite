...
/**
 * REST controller for managing {@link com.vbtn.taskunite.domain.Room}.
 */
@RestController
@RequestMapping("/api")
public class RoomResource {
     ...
     */
    @GetMapping("/rooms")
    public ResponseEntity<List<RoomDTO>> getAllRooms(Pageable pageable, @RequestParam(required = false) String filter) {
        if ("task-is-null".equals(filter)) {
            log.debug("REST request to get all Rooms where task is null");
            return new ResponseEntity<>(roomService.findAllWhereTaskIsNull(),
                    HttpStatus.OK);
        }
