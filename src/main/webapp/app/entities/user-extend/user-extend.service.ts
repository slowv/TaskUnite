...
export class UserExtendService {
  ...
  create(userExtend: IUserExtend): Observable<EntityResponseType> {
    return this.http.post<IUserExtend>(this.resourceUrl, userExtend, { observe: 'response' });
  }

  update(userExtend: IUserExtend): Observable<EntityResponseType> {
    return this.http.put<IUserExtend>(this.resourceUrl, userExtend, { observe: 'response' });
  }

  find(id: number): Observable<EntityResponseType> {
    return this.http.get<IUserExtend>(`${this.resourceUrl}/${id}`, { observe: 'response' });
  }

  query(req?: any): Observable<EntityArrayResponseType> {
    const options = createRequestOption(req);
    return this.http.get<IUserExtend[]>(this.resourceUrl, { params: options, observe: 'response' });
  }
